...
public class QrGeneratorOverviewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        generator = IntStream.range(0, finalGenerator.length).filter(value -> value != 3).mapToObj(value -> finalGenerator[value]).toArray(String[]::new);
        icons = IntStream.range(0, finalIcons.length).filter(value -> value != 3).mapToObj(value -> finalIcons[value]).toArray(Integer[]::new);
